...
You can perhaps think of mewbase as an active database for event driving apps and services. mewbase is a great choice
for event driven microservices.

Mewbase persists events in append only logs. Clients can subscribe to events which are streamed to them from any point in
 the log. Filters can be used to determine which events to subscribe to. In this sense it can act as an event streaming
 engine.

In mewbase all events and documents are BSON objects.

Mewbase also maintains documents (BSON objects), much like a document database, allowing you to query them based on their
